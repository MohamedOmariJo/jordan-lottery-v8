...
def load_data_with_retry() -> Tuple[Optional[pd.DataFrame], str]:
    """تحميل البيانات مع إعادة المحاولة والتسجيل"""
    op_id = logger.start_operation('data_loading', {'source': Config.GITHUB_URL})
    ...
    try:
        with benchmark.monitor_operation('data_loading'):
            ...
            try:
                ...
                logger.logger.warning(f"⚠️ فشل التحميل من الإنترنت: {e}")
                ...
                try:
                    ...
                    error_msg = "❌ لم يتم العثور على ملف البيانات محلياً أو عبر الإنترنت"
                    logger.logger.error(error_msg)
                    logger.end_operation(op_id, 'failed', {'error': error_msg})
                    return None, error_msg
                except Exception as e:
                    error_msg = f"❌ خطأ في قراءة الملف: {e}"
                    logger.logger.error(error_msg)
        ...
        with col_settings:
            with st.form("advanced_generator_form"):
                ...
                if generate_btn:
                    ...
                    # التحقق من القيود
                    is_valid, issues = validator.validate_with_constraints("", constraints)
            ...
            if 'generated_tickets' in st.session_state and st.session_state.generated_tickets:
                ...
                with col_export1:
                    pdf_buffer = PDFGenerator.create_ticket_pdf(
                        tickets,
                        metadata={'strategy': gen_mode, 'constraints': constraints}
                    )
                    st.download_button(
                        "📥 تحميل PDF",
                        pdf_buffer,
    ...
    with tabs[5]:
        ...
        if not st.session_state.portfolio:
            ...
            portfolio_stats = {
                ...
                'preferred_patterns': self._analyze_portfolio_patterns(st.session_state.portfolio)
            }
