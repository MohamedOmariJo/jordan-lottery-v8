...
class PerformanceBenchmark:
    ...
    def stop_monitoring(self, operation_name: str) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            'throughput': 0  # سيتم حسابها لاحقاً
        }
        ...
        if Config.ENABLE_PROFILING:
            ...
            metrics['malloc_increase_mb'] = round((current - start_data['start_malloc']) / 1024 / 1024, 2)
            tracemalloc.stop()
    ...
    def _check_performance_limits(self, operation_name: str, metrics: Dict):
        ...
        if metrics['memory_used_mb'] > Config.MAX_MEMORY_USAGE_MB:
            warnings.append(f"استخدام ذاكرة مرتفع: {metrics['memory_used_mb']}MB")
        
        # التحقق من استخدام CPU
        if metrics['cpu_usage_percent'] > Config.MAX_CPU_PERCENT:
            warnings.append(f"استخدام CPU مرتفع: {metrics['cpu_usage_percent']}%")
    ...
    def _analyze_trends(self, history: List[Dict]) -> Dict[str, Any]:
        ...
        # حساب الميل (إذا كان هناك اتجاه تصاعدي أو تنازلي)
        if len(avg_durations) >= 2:
            ...
            duration_trend = memory_trend = 0
        
        return {
            'days_analyzed': len(days),
            'duration_trend': round(duration_trend, 6),  # موجب = يتزايد، سالب = يتناقص
            'memory_trend': round(memory_trend, 6),
    ...
    def _generate_recommendations(self, history: List[Dict]) -> List[str]:
        ...
        # البحث عن العمليات البطيئة
        slow_operations = []
        for record in history:
            if record['duration_seconds'] > 5.0:  # أكثر من 5 ثواني
                slow_operations.append((record['operation'], record['duration_seconds']))
        ...
        # البحث عن عمليات تستهلك ذاكرة عالية
        memory_hungry = []
        for record in history:
            if record['memory_used_mb'] > 100:  # أكثر من 100MB
                memory_hungry.append((record['operation'], record['memory_used_mb']))
    ...
    def compare_with_baseline(self, baseline_metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        comparison['improvement'] = (
            comparison['duration_change'] < -5 and  # تحسن في الوقت
            comparison['memory_change'] < -10       # تحسن في الذاكرة
        )
        
        comparison['regression'] = (
            comparison['duration_change'] > 10 or   # تدهور في الوقت
            comparison['memory_change'] > 20        # تدهور في الذاكرة
        )
